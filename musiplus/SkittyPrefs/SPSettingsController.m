...
NSString* colourString = [prefs objectForKey:@"fcolor"];
      UIColor* selectedColour = [SparkColourPickerUtils colourWithString: colourString withFallback: @"#ffd3f6"];
              bar.barTintColor = selectedColour;

		bar.tintColor = self.settings[@"textColor"] ?: [UIColor whiteColor];
		if (@available(iOS 13.0, *)) {
			if (self.traitCollection.userInterfaceStyle == UIUserInterfaceStyleDark) {
NSDictionary* prefs1 = [NSDictionary dictionaryWithContentsOfFile: @"/var/mobile/Library/Preferences/com.masonnacs.musiplus.plist"];
NSString* colourString1 = [prefs1 objectForKey:@"fcolor"];
      UIColor* selectedColour1 = [SparkColourPickerUtils colourWithString: colourString1 withFallback: @"#ffd3f6"];
		bar.barTintColor = selectedColour1;
...
- (void)traitCollectionDidChange:(UITraitCollection *)previousTraitCollection {
	if (@available(iOS 13, *)) {
		...
		if (@available(iOS 13.0, *)) {
			if (self.traitCollection.userInterfaceStyle == UIUserInterfaceStyleDark) {
NSDictionary* prefs2 = [NSDictionary dictionaryWithContentsOfFile: @"/var/mobile/Library/Preferences/com.masonnacs.musiplus.plist"];
NSString* colourString2 = [prefs2 objectForKey:@"fcolor"];
      UIColor* selectedColour2 = [SparkColourPickerUtils colourWithString: colourString2 withFallback: @"#ffd3f6"];
bar.barTintColor = selectedColour2;

				if (self.settings[@"darkTextColor"]) bar.tintColor = self.settings[@"darkTextColor"];
			} else {
NSDictionary* prefs3 = [NSDictionary dictionaryWithContentsOfFile: @"/var/mobile/Library/Preferences/com.masonnacs.musiplus.plist"];
NSString* colourString3 = [prefs3 objectForKey:@"fcolor"];
      UIColor* selectedColour3 = [SparkColourPickerUtils colourWithString: colourString3 withFallback: @"#ffd3f6"];
				bar.barTintColor = selectedColour3;
				bar.tintColor = self.settings[@"textColor"] ?: [UIColor whiteColor];
			}
		} else {
NSDictionary* prefs4 = [NSDictionary dictionaryWithContentsOfFile: @"/var/mobile/Library/Preferences/com.masonnacs.musiplus.plist"];
NSString* colourString4 = [prefs4 objectForKey:@"fcolor"];
      UIColor* selectedColour4 = [SparkColourPickerUtils colourWithString: colourString4 withFallback: @"#ffd3f6"];
			bar.barTintColor = selectedColour4;
			bar.tintColor = self.settings[@"textColor"] ?: [UIColor whiteColor];
		}
	}
}
